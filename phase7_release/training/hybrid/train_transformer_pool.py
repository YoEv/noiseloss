...
class HybridCNNTransformerModel(nn.Module):
    def __init__(self, seq_len_in, sae_dim, curve_channels=2, seq_len_out=150, d_model=256, nhead=8, num_layers=4):
        super().__init__()
        self.concat = ConcatProject(curve_channels=curve_channels, sae_dim=sae_dim, d_model=d_model)
        self.seq = CNN1DReduceThenTransformer(
            seq_len_in=seq_len_in, d_in=d_model, seq_len_out=seq_len_out, d_model=d_model, nhead=nhead, num_layers=num_layers
        )

    def forward(self, loss_curve, sae_features):
        return self.seq(self.concat(loss_curve, sae_features))
...
def train_epoch(model, loader, optimizer, criterion, device, max_grad_norm=1.0, scaler=None):
    model.train()
    total = 0.0
    n_ok = 0
    for batch in tqdm(loader, desc="Train", leave=False):
        ...
        total += loss.item()
        n_ok += 1
    return total / n_ok if n_ok else float("nan")
...
def evaluate(model, loader, criterion, device):
    ...
    avg_loss = total / n_ok if n_ok else float("nan")
    p, s = _safe_corr(preds_list, scores_list)
    return avg_loss, p, s, preds_list, scores_list
...
def main():
    ...
    parser.add_argument("--run_name", type=str, default="transformer_cnn_pool")
    parser.add_argument("--seq_len_out", type=int, default=150)
    ...
    tcfg = cfg.get("training", {}).get("defaults", {}).get("hybrid_transformer_pool", {})
    ...
    lr = float(args.lr if args.lr is not None else tcfg.get("lr", 5e-5))
    ...
    model = HybridCNNTransformerModel(
        seq_len, int(train_ds.sae_dim), curve_channels=train_ds.curve_channels, seq_len_out=args.seq_len_out
    ).to(device)
    ...
    ckpt_path = os.path.join(save_dir, f"exp13_hybrid_transformer_cnn_pool_{curve_tag}_{sae_tag}_{run_tag}_best.pth")
    ...
    plt.title(f"Hybrid CNN+Transformer ({curve_tag}+sae) [{sae_tag}] {run_tag}")
    plt.xlabel("Ground Truth")
    plt.ylabel("Predicted")
    plt.savefig(os.path.join(plot_dir, f"exp13_hybrid_transformer_cnn_pool_{curve_tag}_{sae_tag}_{run_tag}_scatter.png"), dpi=150)
    plt.close()
