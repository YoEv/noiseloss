...
def get_song_name_from_filename(filename):
    ...
    """
    # Remove the prefix 'loss_diff_' and suffix with variable token range
    song_name = filename.replace('loss_diff_', '')
    # Use regex to remove the token range and statistics suffix
    import re
    song_name = re.sub(r'_tokens_\d+_\d+_statistics\.txt$', '', song_name)
    # Further clean the name
    song_name = song_name.replace('_short30_preview_cut14s.mp3', '')
    return song_name
...
def collect_data_from_directories():
    ...
    categories = ['tk5', 'tk10', 'tk20', 'tk50', 'tk100', 'tk200']
    
    # Dictionary to store data: {song_name: {category: {'mean': value, 'std': value}}}
    data = {}
    
    for category in categories:
        dir_path = os.path.join(base_dir, f'loss_diff_plot_unconditional_{category}')
        
        if not os.path.exists(dir_path):
            print(f"Warning: Directory {dir_path} does not exist")
            continue
        
        # Navigate into subdirectories to find statistics files
        for subdir in os.listdir(dir_path):
            subdir_path = os.path.join(dir_path, subdir)
            if os.path.isdir(subdir_path):
                # Look for statistics files in subdirectory
                stats_files = [f for f in os.listdir(subdir_path) if f.endswith('_statistics.txt')]
                
                for stats_file in stats_files:
                    file_path = os.path.join(subdir_path, stats_file)
                    song_name = get_song_name_from_filename(stats_file)
...
def create_comparison_plots():
    ...
    # Save the plot
    output_file = '/home/evev/asap-dataset/Plot/unconditional_token_length_comparison.png'
    ...
    token_lengths = [5, 10, 20, 50, 100]  # Numeric values for plotting
    mean_of_means = []
    ...
    # Save the mean plot
    mean_output_file = '/home/evev/asap-dataset/Plot/unconditional_mean_by_token_length.png'
    ...
    # Save the box plot
    box_output_file = '/home/evev/asap-dataset/Plot/unconditional_boxplot_by_token_length.png'
    ...
    # Set y-axis maximum to 140
    ax4.set_ylim(0, 140)
    
    plt.tight_layout()
    
    # Save the count plot
    count_output_file = '/home/evev/asap-dataset/Plot/unconditional_count_by_token_length.png'
    plt.savefig(count_output_file, dpi=300, bbox_inches='tight')
    print(f"Count plot saved to {count_output_file}")
    
    # Save summary statistics
    summary_file = '/home/evev/asap-dataset/Plot/unconditional_token_length_summary.txt'
    with open(summary_file, 'w') as f:
        f.write("UNCONDITIONAL TOKEN LENGTH COMPARISON SUMMARY\n")
        f.write("="*50 + "\n\n")
    ...
    csv_file = '/home/evev/asap-dataset/Plot/unconditional_token_length_data.csv'
