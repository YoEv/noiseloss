...
class MidiModifier:
    def __init__(self, log_txt_path):
        self.log = []
        self.error_log = []
        self.log_file = open(log_txt_path, 'w', encoding='utf-8')
    ...
    def modify_single_note(self, midi_path, output_path):
        try:
            ...
            valid_notes = [n for n in selected_chord if 12 <= n.pitch <= 115]
            if not valid_notes:
                return False
            ...
            for _ in range(20):
                ...
                if self._validate_pitch(mod_midi, original_pitch, new_pitch, selected_chord[0].pitch % 12):
            ...
            error_msg = f"处理错误 {midi_path}: {str(e)}"
    ...
    def modify_voicing(self, midi_path, output_path):
        try:
            ...
            chord_notes = sorted(valid_notes, key=lambda x: x.pitch)
...
if __name__ == "__main__":
    modifier = MidiModifier(log_txt_path="./modification_log.txt")
