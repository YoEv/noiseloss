...
class SoundStream(nn.Module):
    ...
    def __init__(
        ...
        if self.is_semantic:
            # 将相对路径改为“相对于本文件”的绝对路径，且只走本地加载
            base_dir = os.path.dirname(__file__)
            ckpt_dir = os.path.abspath(os.path.join(base_dir, "..", "semantic_ckpts", "hf_1_325000"))
            self.semantic_model = AutoModel.from_pretrained(ckpt_dir, local_files_only=True)
