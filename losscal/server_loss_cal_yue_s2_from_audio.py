...
import sys
sys.path.append('.')
sys.path.append('./external/YuEGP/inference')
sys.path.append('./external/YuEGP/inference/xcodec_mini_infer')
...
class XCodecAudioTokenizer:
    ...
    def _load_xcodec_model(self):
        ...
        try:
            ...
            model_path = './external/YuEGP/inference/xcodec_mini_infer/final_ckpt/ckpt_00360000.pth'
...
def parse_args():
    parser = argparse.ArgumentParser(description="Calculate loss using YuEGP-s2 model from audio files")
    parser.add_argument("--audio_dir", type=str, default="+Dataset/Phase5_1", help="Directory containing audio files")
    parser.add_argument("--output_dir", type=str, default="test_out", help="Output directory for results")
    parser.add_argument("--model_name_or_path", type=str, default="m-a-p/YuEGP-s2-1B-general", help="Model name or path")
    ...
    parser.add_argument("--token_output_dir", type=str, help="Directory to save per-token CSV files (default: output_dir/per_token)")
...
def process_single_audio(audio_path, tokenizer_model, yuegp_model, device, token_output_dir=None, relative_path=None):
    try:
        # 清理GPU缓存
        if device != "cpu":
        ...
        if len(token_ids) > max_seq_length:
            ...
            for i in range(0, len(token_ids), step):
                ...
                if len(chunk_ids) < 2:
                    continue
                chunk_nll = compute_per_token_nll(yuegp_model, chunk_ids, device)
            ...
            # 正常处理短序列
            nll = compute_per_token_nll(yuegp_model, token_ids, device)
            avg_loss = nll.mean().item()
...
def process_audio_directory(audio_dir, tokenizer_model, yuegp_model, device, output_file=None, token_output_dir=None, batch_interval=5):
    audio_files = []
    ...
    for i, rel_filename in enumerate(tqdm(audio_files, desc="Processing")):
        ...
        loss = process_single_audio(audio_path, tokenizer_model, yuegp_model, device, token_output_dir, rel_filename)
...
def main():
    ...
    if args.save_per_token:
        token_output_dir = args.token_output_dir if args.token_output_dir else os.path.join(args.output_dir, "per_token")
        print(f"Per-token CSV files will be saved to: {token_output_dir}")
    ...
    if results:
        ...
        if token_output_dir:
            print(f"Per-token CSV files saved to: {token_output_dir}")
    else:
