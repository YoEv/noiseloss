...
def perform_statistical_tests(category_data):
    """执行统计检验"""
    # ANOVA检验（三组比较）
    salamander_data = [category_data[cat] for cat in TARGET_CATEGORIES if 'Salamander' in cat]
    ydp_data = [category_data[cat] for cat in TARGET_CATEGORIES if 'YDP' in cat]

    # 乐器内部比较
    anova_salamander = stats.f_oneway(*salamander_data)
    anova_ydp = stats.f_oneway(*ydp_data)

    # 乐器间比较（相同action类型）
    t_test_results = []
    for action in ['original', 'action1', 'action2']:
        sal_data = category_data[f"{action}_Salamander"]
        ydp_data = category_data[f"{action}_YDP"]
        t_stat, p_val = stats.ttest_ind(sal_data, ydp_data)
        t_test_results.append({
            'Comparison': f"{action}_Salamander vs {action}_YDP",
            't-statistic': t_stat,
            'p-value': p_val,
            'Significant (97%)': p_val < 0.03
        })

    return {
        'ANOVA_Salamander': anova_salamander,
        'ANOVA_YDP': anova_ydp,
...
if __name__ == "__main__":
    ...
    print("\nANOVA Results for Salamander Groups:")
    print(f"F = {test_results['ANOVA_Salamander'].statistic:.3f}, p = {test_results['ANOVA_Salamander'].pvalue:.4f}")

    print("\nANOVA Results for YDP Groups:")
    print(f"F = {test_results['ANOVA_YDP'].statistic:.3f}, p = {test_results['ANOVA_YDP'].pvalue:.4f}")
