...
def extract_mean_std_from_file(file_path):
    """Extract mean and std from statistics file"""
    try:
        with open(file_path, 'r') as f:
            content = f.read()
        
        # Find the Loss Difference Statistics section
        pattern = r'Loss Difference Statistics \(File2 - File1, Noise Region\):\s*Mean: ([\d\.\-]+)\s*Std: ([\d\.\-]+)'
        ...
        if match:
            ...
            print(f"Could not find loss difference statistics in {file_path}")
            return None, None
...
def get_song_name_from_filename(filename):
    ...
    # Remove the statistics file suffix and extract the base song name
    song_name = filename.replace('loss_diff_', '')
    # Use regex to remove the tokens part and statistics suffix
    song_name = re.sub(r'_tokens_\d+_\d+_statistics\.txt$', '', song_name)
    # Further clean the name
    song_name = song_name.replace('_ori_cut15s', '')
    return song_name

def collect_data_from_directories():
    """Collect data from all directories"""
    base_dir = '/home/evev/asap-dataset/Loss_Plot/Phase4_3/loss_diff_plot_asap_tk'
    ...
    for category in categories:
        ...
        for subdir in os.listdir(dir_path):
            ...
            if os.path.isdir(subdir_path):
                # Look for statistics files in each subdirectory
                for filename in os.listdir(subdir_path):
                    if filename.endswith('_statistics.txt'):
                        file_path = os.path.join(subdir_path, filename)
                        song_name = get_song_name_from_filename(filename)
                        
                        mean, std = extract_mean_std_from_file(file_path)
                        if mean is not None and std is not None:
                            if song_name not in data:
                                data[song_name] = {}
                            data[song_name][f'tk{category}'] = {'mean': mean, 'std': std}
    
    return data, [f'tk{cat}' for cat in categories]
...
def create_comparison_plots():
    ...
    # Create plots with same style as shutter
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(16, 12))
    
    # Plot 1: Mean Loss Difference
    x = np.arange(len(complete_songs))
    
    # Use same colors as shutter script
    ...
    ax1.set_title('Mean Loss Difference Statistics (File2 - File1, Noise Region) Across Token Lengths - ASAP', fontsize=14)
    ax1.set_xticks(x)
    ...
    ax2.set_title('Standard Deviation of Loss Difference Statistics (File2 - File1, Noise Region) Across Token Lengths - ASAP', fontsize=14)
    ax2.set_xticks(x)
    ...
    # Save the plot
    output_file = '/home/evev/asap-dataset/Plot/asap_token_length_comparison.png'
    ...
    # Plot the mean of means with same style as shutter
    ...
    ax3.set_title('Average Loss Difference Across All ASAP Songs by Token Length', fontsize=14)
    ...
    # Save the mean plot
    mean_output_file = '/home/evev/asap-dataset/Plot/asap_mean_by_token_length.png'
    ...
    # Save the box plot
    box_output_file = '/home/evev/asap-dataset/Plot/asap_boxplot_by_token_length.png'
    ...
    # Create bar plot with same style as shutter
    ...
    ax4.set_title('Count of ASAP Songs with Positive vs Negative Mean Loss Difference by Token Length', fontsize=14)
    ax4.set_xticks(x)
    ...
    # Save the count plot
    count_output_file = '/home/evev/asap-dataset/Plot/asap_count_by_token_length.png'
    plt.savefig(count_output_file, dpi=300, bbox_inches='tight')
    print(f"Count plot saved to {count_output_file}")
    
    # Save summary statistics
    summary_file = '/home/evev/asap-dataset/Plot/asap_token_length_summary.txt'
    with open(summary_file, 'w') as f:
        f.write("ASAP TOKEN LENGTH COMPARISON SUMMARY\n")
        f.write("="*50 + "\n\n")
    ...
    csv_file = '/home/evev/asap-dataset/Plot/asap_token_length_data.csv'
