...
def crop_midi(midi, start_beat=0, num_bars=None):
    """智能裁剪MIDI片段"""
    # 如果未指定小节数，随机选择8-16小节
    if num_bars is None:
...
if __name__ == "__main__":
    # 直接从ASAP_BASE目录获取所有MIDI文件
    midi_files = glob.glob(os.path.join(ASAP_BASE, "*.mid"))

    print(f"找到 {len(midi_files)} 个MIDI文件在 {ASAP_BASE} 目录")
    ...
    print(f"其中 {len(valid_files)} 个MIDI文件有效")

    # 随机选择100个（或全部不足时）
    selected = random.sample(valid_files, min(100, len(valid_files)))

    # 处理并保存MIDI
    success_count = 0
    for idx, midi_path in enumerate(selected):
        try:
            midi = load_and_validate_midi(midi_path)
            if not midi:
                continue

            # 随机选择8-16小节长度
            bars_length = random.randint(8, 16)

            # 裁剪MIDI
            cropped = crop_midi(midi, num_bars=bars_length)

            # 生成输出文件名
            filename = Path(midi_path).stem
            output_path = os.path.join(OUTPUT_DIR, f"{filename}_short.mid")

            # 保存并验证
            cropped.write(output_path)
            if os.path.exists(output_path):
                success_count += 1
                print(f"✅ Saved: {output_path} ({len(cropped.instruments[0].notes)} notes, {bars_length} bars)")
            else:
                print(f"❌ Failed to save: {output_path}")

        except Exception as e:
            print(f"⚠️ Error processing {midi_path}: {str(e)}")
    ...
    print("\n" + "="*50)
    print(f"处理完成！成功保存 {success_count}/{len(selected)} 个短MIDI文件")
